#!/usr/bin/env python
"""
VARIANT H -- corpus v2_4_briefclean. ONE VARIABLE CHANGED vs. variant G: the
corpus (v2_3_ch3x minus the 85 briefs whose target first-word reuse exceeds the
chapter branch's p90 of 29.2: 414 chapter + 479 brief = 893, sha256[:16]
b092e24d5cf9a0fd, built by build_corpus_v2_4_briefclean.py). Tests whether the
sentence-shape prior (sixteenth run, addendum) lives in the brief branch's
high-reuse tail. Hyperparameters identical to G; save_steps 28 keeps the
half-epoch checkpoint cadence at the smaller corpus (893*2/16 ~ 112 steps).
Prediction pre-registered in logs/prediction_variantH.txt BEFORE this ran.

--- original variant-G header follows ---
...
DATASET_PATH     = "/workspace/final_training_corpus_v2_4_briefclean.json"  # VARIANT H: the one change (brief tail > p90 dropped)
...
SEED                   = 3407
OUTPUT_DIR             = "/workspace/drift_sft_out_v8"  # VARIANT H: separate dir
LOGGING_STEPS          = 1
...
def main():
    ...
    training_args = SFTConfig(
        ...
        save_steps=28,          # half-epoch at 893 examples (893*2/16 ~ 112 steps)
    ...
    print("\n" + "=" * 70)
    print("CONFIG SUMMARY -- VARIANT H (corpus v2_4_briefclean)")
    print("=" * 70)
    ...
    if RUN_TRAINING:
        ...
        print("\n" + "#" * 70)
        print("TRAINING COMPLETE -- VARIANT H")
        print("#" * 70)
